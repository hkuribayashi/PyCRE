...
from rl.qlearning.action import ActionSpace
from rl.qlearning.observation import ObservationSpace


class Environment:

    def __init__(self, slice_):
        self.slice = copy.deepcopy(slice_)
        self.working_slice = copy.deepcopy(slice_)
        self.action_space = ActionSpace(len(self.slice.selected_bs) * 7)
        self.observation_space = ObservationSpace(101)
        self.priority_ues_weight = self.slice.cluster.bs_list[-1].hetnet.env.priority_ues_weight
        self.ordinary_ues_weight = self.slice.cluster.bs_list[-1].hetnet.env.ordinary_ues_weight

    def reset(self):
        self.working_slice = copy.deepcopy(self.slice)
        state = int(self.slice.cluster.evaluation['satisfaction'])
        return state

    def compute_satisfaction(self):
        fulfilled_qos_ues = np.array([ue for ue in self.slice.cluster.ue_list if ue.evaluation is True])
        total_priority_ues = len([ue for ue in self.slice.cluster.ue_list if ue.priority is True])
        total_ordinary_ues = len([ue for ue in self.slice.cluster.ue_list if ue.priority is False])
    ...
    def step(self, action, state):
        ...
        target_bs = self.slice.selected_bs[bs_index]
